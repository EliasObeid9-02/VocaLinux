...
#ar_std([
  ...
  #figure(
    table(
      ...
      [*الفئة*], [*المكتبة/الأداة*], [*الاستخدام في المشروع*],
...
#ar_std([
  يتم تحويل أشكال الموجات الصوتية الخام إلى *مخططات طيفية #en_clean_std([Log-Mel])*، وهي تمثيل للميزات يحاكي الإدراك السمعي البشري ويكون فعالاً للغاية في مهام التعرف على الكلام. يتبع هذا التحويل الخطوات:
...
#ar_std([
  ...
  - *إنشاء مترجم أوامر قوي*: تم تطوير مترجم فوري متطور قائم على القواعد لترجمة مخرجات النص الخام لنموذج #en_clean_std([ASR]) إلى أوامر صالحة. وقد أثبتت عمليته ذات المرحلتين، التي تضم أداة ترجمة الكلمات الرئيسية التي تعمل عن طريق إيجاد أطول مطابقة ومحلل مسار الذي يعمل عن طريق المطابقة الضبابية، فعاليتها في التعامل مع غموض اللغة المنطوقة وتصحيح أخطاء التدوين البسيطة، وبالتالي زيادة قابلية استخدام النظام بشكل عام.
...
#ar_std([
  واستناداً إلى النتائج المحققة والصعوبات التي تمت مواجهتها، تم تحديد عدة اتجاهات واضحة للعمل المستقبلي. وتهدف هذه التوصيات إلى البناء على النجاحات التي حققها المشروع ومعالجة قيوده الحالية بشكل مباشر.
  - *توسيع مجموعة بيانات التدريب*: تتمثل الخطوة التالية الأكثر أهمية في معالجة مشكلة الإفراط في الملاءمة عن طريق زيادة حجم بيانات التدريب بشكل كبير. إن الانتقال من مجموعة `train-clean-100` (#en_clean_std([$100$]) ساعة) إلى مجموعة `train-clean-360` (#en_clean_std([$360$]) ساعة) أو `train-other-500` (#en_clean_std([$500$]) ساعة) من #en_clean_std([LibriSpeech]) من شأنه أن يوفر للنموذج مجموعة بيانات أكثر ثراءً وتنوعًا، مما يعطي مبررًا أفضل لحجم شبكته الكبير ويؤدي على الأرجح إلى تحسينات كبيرة في التعميم والدقة الإجمالية.
  - *تجربة تغيير حجم النموذج وتعديل البيانات*: كنهج بديل أو مكمّل لتوسيع مجموعة البيانات، يجب أن يستكشف العمل المستقبلي تقليل حجم النموذج. يمكن أن يؤدي التقليل المنهجي لعدد وحدات #en_clean_std([LSTM]) أو طبقات #en_clean_std([pBLSTM]) إلى إنشاء شبكة ذات حجم أكثر ملاءمة لمجموعة بيانات #en_clean_std([$100$]) ساعة، مما قد يخفف من #en_clean_std([overfitting]). وعلاوة على ذلك، يمكن دراسة تطبيق أكثر قوة ل #en_clean_std([SpecAugment]) لتعزيز تنوع بيانات التدريب الحالية.
  - *تعزيز المترجم القائم على القواعد*: على الرغم من أن المترجم التفسيري وظيفي، إلا أنه يمكن توسيعه لدعم مجموعة أوسع من الأوامر. يجب أن تركز التعديلات المستقبلية على ما يلي:
    - *دعم ميزات #en_clean_std([shell]) المتقدمة*: تطوير النظام للتعامل مع تسلسل الأوامر (`|`)، وإعادة توجيه المخرجات (`<`)، والمدخلات المعقدة سيزيد من قوة النظام وفائدته بشكل كبير.
    - *المفردات الواعية بالسياق*: يمكن جعل المترجم أكثر ذكاءً من خلال الحفاظ على الوعي بمجلد العمل الحالي. وهذا من شأنه أن يسمح له بتقديم إكمال أكثر دقة للمسار واقتراح وسيطات صالحة للأوامر مثل ’cd‘ بشكل ديناميكي.
  - *التكيف والضبط الدقيق الخاص بالمستخدم*: لزيادة تحسين إمكانية الوصول، يمكن تطوير نظام للضبط الدقيق الخاص بالمستخدم. ومن المرجح أن يؤدي السماح للنموذج بالتكيف مع لهجة المستخدم ومفرداته والأوامر الشائعة الاستخدام إلى تعزيز كبير في الأداء في العالم الحقيقي ورضا المستخدم.
])

...
#ar_std([
  ...
  #figure(
    ...
    kind: image,
    caption: flex_captions(
      [البنية الشاملة لنموذج الاستماع والمتابعة والتهجئة (#en_clean_std([LAS]))، والتي توضح التدفق من المستمع إلى المهجئ.],
...
#ar_std([
  ...
  - *إجبار المعلم*: توفر طريقة التدريب المعيارية الرمز السابق الحقيقي، #en_clean_std([$y_(t-1)$]), كمدخلات عند توقع الرمز التالي، #en_clean_std([$y_t$]). هذه الطريقة فعالة ومستقرة، ولكنها تخلق عدم تطابق مع الاستدلال، حيث لا يتوفر الرمز الحقيقي. لا يتم تدريب النموذج أبدًا على التعافي من أخطائه.

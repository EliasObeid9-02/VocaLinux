...
class Speller(Layer):
    ...
    def __init__(
        ...
        self.kernel_init = tf.keras.initializers.RandomUniform(minval=-0.1, maxval=0.1)
        self.recurrent_init = tf.keras.initializers.RandomUniform(minval=-0.1, maxval=0.1)

        self._sampling_probability = tf.Variable(
            sampling_probability,  # type: ignore
            dtype=tf.float32,
            trainable=False,
            name="sampling_probability_var",
        )
    ...
    def build(self, input_shape: List[tf.TensorShape]) -> None:
        ...
        self.embedding = Embedding(
            ...
            embeddings_initializer=self.kernel_init,
        )
            ...
            char_id_for_embedding = tf.cond(
                ...
                true_fn=lambda: tf.cond(
                    tf.logical_and(
                        ...
                        tf.random.uniform([], minval=0.0, maxval=1.0) < self._sampling_probability,
                    ),
